...
Mat PathChaser::roadline(Mat frame) {
    ...
    // clustering 2
    vector<vector<Point2f>> clusters2 = this->groupclusters(clusters, couples, vtangles, 2.75);
    ...
    for (int i = 0; i < pvt.size(); i++) {
        for (int j = 0; j < pvt.size() - 1; j++) {
            ...
            */
            ...
            double kp = a1 / a2;
            if (kp <= this->clsAgliL || kp >= this->clsAgliH) continue;
...
void PathChaser::video(string v, int wk) {
    VideoCapture cap(v);
    cap.set(CV_CAP_PROP_POS_FRAMES, this->frs + 1);
...
void PathChaser::settrackbar() {
    ...
    createTrackbar("Used line segments", this->dbwn4, &this->cpLimFl, 50, PathChaser::debugger, this);
    #endif
